...
# revised version for the data generation,
## output both ctc-sequence and non ctc-sequence
    ...
    for indx, segment_length in enumerate(raw_label.length):
        ...
        if current_length + segment_length < max_seq_length:
            ...
            if current_length > (max_seq_length / 2) and len(current_label) > 5:
                """
                # old padding algorithm
                padding(current_event, max_seq_length,
                        raw_signal[current_start + segment_length:current_start + segment_length + max_seq_length])

                padding(current_label_vec, max_seq_length, [-1]*(max_seq_length-current_length))
                """
       		# no signal part
                padding(current_event,     max_seq_length, [  0 ]*(max_seq_length-current_length))
                padding(current_label_vec, max_seq_length, [  4 ]*(max_seq_length-current_length))
