...
def train():
    ...
    trainY = np.zeros((trainX.shape[0], trainX.shape[1]), dtype=np.float32)

    for i in range(trainY.shape[0]):
        split = np.cumsum(label_seg[i])[:-1]
        trainY[i, split] = 1
    ...
    #######################################
    kernel_size =[128, 128]
    maxpooling_len = [5, 5, 4, 4, 5, 5]
    conv_window_len = 7
    dropoutRate = 0.1
    lr = 0.001

    signals = Input(name='input', shape=[trainX.shape[1], 1], dtype=np.float32)
    gru_1 = layers.GRU(64, return_sequences=True, kernel_initializer='he_normal')(signals)
    gru_1b = layers.GRU(64, return_sequences=True, go_backwards=True, kernel_initializer='he_normal')(signals)
    gru1_merged = layers.Concatenate()([gru_1, gru_1b])

    inner = layers.Dense(trainY.shape[-1])(gru1_merged)
    y_pred = layers.Activation('softmax')(inner)

    model = models.Model(inputs=[signals], outputs=y_pred)
    ...
    fig=plt.figure()
    figureSavePath="../experiment/devLOG/UNet-train_curve_" + lossType + ".png"
